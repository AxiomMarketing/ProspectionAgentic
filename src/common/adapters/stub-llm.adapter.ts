...
import { ILlmAdapter, LlmCompletionRequest, LlmCompletionResponse } from '@common/ports/i-llm.adapter';
...
export class StubLlmAdapter extends ILlmAdapter {
  ...
  async complete(request: LlmCompletionRequest): Promise<LlmCompletionResponse> {
    this.logger.warn({ msg: 'StubLlmAdapter: LLM not configured, returning placeholder response', model: request.model });
    return {
